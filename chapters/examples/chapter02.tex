...
In dieser Arbeit werden die AWS-Lambda-Funktionen für alle fünf Benchmarks, asynchron über Events 
aufgerufen. In diesem Fall werden die eingehenden Events in einer internen asnychronen 
Warteschlange (Event Buffer) gespeichert, bis sie von der Funktion abgeholt 
und verarbeitet werden~\cite{AWSLambdaAsyncDelays}. Dabei sind alle 3 Komponenten der 
\ac{EDA} vertreten. Der Aufrufer fungiert als Event Producer, die Warteschlange als 
Event Broker (Event Ingestion) und die Lambda-Funktion als Event Consumer.
...
\begin{itemize}
    \item \textbf{Ereignisgesteuerte Ausführung:} Funktionen werden nur bei Bedarf aktiviert und verbrauchen im Leerlauf keine Ressourcen. Das beeinflusst die Messung von sowohl Warmstarts als auch Kaltstarts für alle  zu messenden Metriken (siehe Kapitel \ref{ch:chapter04}).
...
\subsection{Zielsetzung von OpenTelemetry}
...
\begin{figure}[H]
 \centering
 \includegraphics[width=1.0\textwidth]{gfx/examples/otel-diagram.pdf}
 \caption{OpenTelemetry~\cite{OpenTelemetryDocs}}
 \label{fig:chapter02:otel}
\end{figure}
Abbildung~\ref{fig:chapter02:otel} zeigt alle Anschlussmöglichkeiten, wie Tracing-Daten an OpenTelemetry und von OpenTelemetry 
...
\begin{figure}[H]
 \centering
 \includegraphics[width=1.0\textwidth]{gfx/examples/traces-venn-diagram.pdf}
 \caption{Samplingkriterien~\cite{OpenTelemetrySampling}}
 \label{fig:chapter02:sampling}
\end{figure}
Abbildung~\ref{fig:chapter02:sampling} veranschaulicht, nach welchen Kriterien, Entscheidungen über das Sampling getroffen werden können.
...
In manchen Systemen werden Head- und Tail-Sampling kombiniert um das Volumen der Tracing-Daten zu reduzieren, die Kosten zu senken und die Telemetrie-Pipeline zu entlasten~\cite{OpenTelemetrySampling}.

Für die Messungen in dieser Arbeit wird Head Sampling mit einer Erfassungsrate von 100\% verwendet. Die vorliegende 
Arbeit hat das Ziel, möglichst viel Daten zu erfassen um den Overhead möglichst genau zu messen. Eine Effizienzoptimierung ist nicht Teil der Evaluierung. 

Die zuvor beschriebenen Aspekte – Kriterien für die Entscheidung, 
ob Sampling zum Einsatz kommt, verschiedene Sampling-Strategien sowie deren Kombination 
– verdeutlichen, dass der durch Tracing verursachte Overhead ein zentrales Thema darstellt, 
das mitunter erhebliche wirtschaftliche Konsequenzen haben kann.
...
In dieser Arbeit wird der ADOT-Layer für Java, Python und Node.js verwendet, um die Funktionen mit zu instrumentieren. Als Backend dient AWS X-Ray, da es nativ mit Lambda integriert ist und die gesammelten Traces in CloudWatch verfügbar macht.
...
\subsection{Relevante Kernfunktionen}
