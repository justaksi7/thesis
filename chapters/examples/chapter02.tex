...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.4\textwidth]{gfx/examples/Cloud_Computing.png}
...
Eine besondere Ausprägung von PaaS ist das \ac{FaaS}-Modell, bei dem Entwickler einzelne Funktionen 
...
Für diese Arbeit ist das Zusammenspiel beider Konzepte zentral, weil die zu untersuchenden AWS-Lambda-Funktionen typischerweise als Event-Consumer oder -Producer in einer ereignisgesteuerten Microservices-Umgebung agieren~\cite{AWSLambdaDocs} 
und die Kommunikation zwischen ihnen (z. B. eine Lambda-Funktion ruft eine andere auf) asynchron über Events erfolgt.
...
dass die anderen Dienste beeinflusst werden~\cite{Fowler2024Microservices}. Dieser Architekturstil ist für diese Arbeit relevant, 
da die Serverless-Technologien und die FaaS-Platformen spezifisch für die Entwicklung von Microservices konzipiert sind (siehe Abschnitt~\ref{sec:serverless}). 
Zudem sind die 5 Microbenchmarks typische Beispiele für Microservices - klar abgegrenzte Funktionen, die jeweils eine spezifische Aufgabe erfüllen.

\begin{figure}[H]
 \centering
 \includegraphics[width=0.6\textwidth]{gfx/examples/microservices.jpg}
...
empfängt, speichert und an die entsprechenden Event Consumer weiterleitet. Beispliele für Event Broker sind AWS \ac{SNS}, \ac{SQS} usw. oder 
...
\textit{Event Consumern} abgeholt wird~\cite{AWSEventDrivenArchitecture,AzureEventDrivenArchitecture}. 
...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.9\textwidth]{gfx/examples/EDA.png}
...
können~\cite{AWSLambdaDocs} und damit in ihrem Kern ereignisgesteuert sind.
...
Für diese Arbeit ist das \ac{FaaS}-Modell zentral, weil es als die am weitesten verbreitete Form von Serverless gilt und die AWS-Lambda-Funktionen genau diesem Modell folgen~\cite{ServerlessAWSLambda}.
...
\begin{itemize}
    \item \textbf{Ereignisgesteuerte Ausführung:} Funktionen werden nur bei Bedarf aktiviert und verbrauchen im Leerlauf keine Ressourcen. Das beeinflusst die Messung von sowohl Warmstarts als auch Kaltstarts für alle  zu messenden Metriken (siehe Kapitel 4).
